...
O comando SQL {\tt INSERT} indica que tabela estamos utilizando, e em seguida, 
...
%Then we use the {\tt SELECT} command to retrieve the rows we just inserted
%from the table. On the {\tt SELECT} command, we indicate which columns we
%would like {\tt (title, plays)} and indicate which table we want to retrieve
%the data from. After we execute the {\tt SELECT} statement, the cursor is
%something we can loop through in a {\tt for} statement. For efficiency, the
%cursor does not read all of the data from the database when we execute the
%{\tt SELECT} statement. Instead, the data is read on demand as we loop through
%the rows in the {\tt for} statement.
